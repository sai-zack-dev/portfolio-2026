...
export type TechRow = { layer: string; tech: string };
export type ApiEndpoint = { method: string; path: string; description: string };
export type InstallStep = { title: string; code?: string; note?: string };
export type DownloadPlatform = {
  platform: string;
  label: string;
  href: string;
};
export type FuturePlan = string;
        ...
        endpoints: [
          {
            method: "GET",
            path: "/",
            description: "Health check - returns a random greeting",
          },
          {
            method: "GET",
            path: "/greet",
            description: "Returns a random opening greeting",
          },
          {
            method: "POST",
            path: "/chat",
            description: "Send a message, receive reply + emotion analysis",
          },
          {
            method: "POST",
            path: "/emotion",
            description: "Emotion analysis only, no bot reply",
          },
        ],
      },
      {
...
# Response
{
...

  {
    ...
    nda: true,
    ndaNote: "Private client project under confidentiality agreement.",
    highlights: [
    ...
    readme: [
      ...
      },
      {
        type: "note",
        content:
          "Developed for a private client under confidentiality agreement. Source code and project details remain private.",
      },
    ],
  },

  {
    ...
    nda: true,
    ndaNote: "Private client projects under confidentiality agreements.",
    highlights: [
    ...
    readme: [
      ...
      },
      {
        type: "note",
        content:
          "Developed for private clients under confidentiality agreements. Source code and client details remain confidential.",
      },
    ],
  },
...
// ─── Categories ───────────────────────────────────────────────────────────────
// projectIds reference the ids above - used by list.tsx to group cards.
...
export const categories: Category[] = [
  {
    ...
    label: "AI Projects",
    icon: "⬡",
    projectIds: ["mm-hw-recognizer", "qbiv", "elyza", "mm-hw-collector"],
  },
  {
    key: "design",
    label: "Design Projects",
    icon: "◈",
    projectIds: ["pocket-dev-env", "flat-sync", "say-cheese", "easy-clean"],
  },
  {
    key: "professional",
...
export function getRelatedProjects(current: Project, max = 3): Project[] {
  return projects
...
export function getCategoriesWithProjects(): (Category & {
  projects: Project[];
})[] {
  return categories.map((cat) => ({
    ...cat,
    projects: cat.projectIds.map((id) => getProjectById(id)!).filter(Boolean),
  }));
}
